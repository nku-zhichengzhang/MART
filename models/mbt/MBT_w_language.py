...
from .VanillaViT import vit_base_patch16_112 as VideoMAE



class MBT_w_language(nn.Module):
    def __init__(self, n_classes,audio_time=100,r_act=4):
        super(MBT_w_language, self).__init__()
    ...
    def bottleneck_fusion(self, visual, audio, bottles, extractFea=False):
        bv,tv,_ = visual.size()
        _,ta,_ = audio.size()
        _,tb,_ = bottles.size()
    ...
    def forward_bottleneck_cls(self, visual, audio, language, extractFea=False):
        visual = visual.transpose(0, 1).contiguous()
        visual.div_(self.NORM_VALUE).sub_(self.MEAN)
        ...
        language_feature = language_feature.view(batch, seq_len, -1)
        temporal_t = language_feature
        language_feature = torch.mean(language_feature, dim=1)    
        
        
        # fc classifier
        output_a = self.a_fc(audio_feature)
        output_v = self.v_fc(visual_feature)
        output_t = self.t_fc(language_feature)

        
        with torch.no_grad():
            temporal_score_v = self.v_fc(temporal_v)  # [bs, seqlen, class]
            temporal_score_a = self.a_fc(temporal_a)  # [bs, seqlen, class]
            temporal_score_t = self.t_fc(temporal_t)  # [bs, seqlen, class]
            
            temporal_score = temporal_score_v + temporal_score_a + temporal_score_t
        
        return [output_a, output_v, output_t], temporal_score
    ...
    def forward_bottleneck_w_VMask_att(self, visual, audio):
        ...
        assert audio_feature.shape[0] == visual_feature.shape[0]
        bottles = self.bottleneck.repeat(audio_feature.shape[0],1,1)
        visual_feature, attn = self.bottleneck_fusion_cls_attn(visual_feature, audio_feature, bottles, extractFea=True)
        # if extractFea:
        return visual_feature, attn
    ...
    def forward(self, visual, audio, words, type='mbt'):
        return self.forward_bottleneck_cls(visual, audio, words)
